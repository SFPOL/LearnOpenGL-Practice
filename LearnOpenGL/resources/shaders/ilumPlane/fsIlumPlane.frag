...
vec3 CalcPointLight(PntLight light, vec3 normal, vec3 fragPos, vec3 viewDir, vec3 materialDiffuse, vec3 materialSpecular, bool blinn) {
...
//    vec3 result = CalcDirLight(dirLight, norm, viewDir, materialDiffuse, materialSpecular);
    ...
    for(int i = 0; i < NR_POINT_LIGHTS; i++)
        result += CalcPointLight(pointLights[i], norm, FragPos, viewDir, materialDiffuse, materialSpecular, blinn);
